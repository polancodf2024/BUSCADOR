...
# ============================================================================
# FUNCIONES DE UTILIDAD PARA PubMed CON MANEJO DE RATE LIMITING MEJORADO
# ============================================================================
...
# ============================================================================
# EXTRACCIÓN DINÁMICA DE TÉRMINOS
# ============================================================================
...
def extract_medical_entities_enhanced(text, query_terms, hypothesis_terms):
    """Extract entities combining embeddings and regex"""
    if not text:
        return []
...
def search_pubmed(query, retmax=1000):
    """Search articles in PubMed with rate limiting - limited to 1000 articles"""
    base_url = "https://eutils.ncbi.nlm.nih.gov/entrez/eutils/"
...
def fetch_articles_details(id_list, query_terms, hypothesis_terms):
        ...
        for retry in range(max_retries):
            try:
                if retry > 0:
                    ...
                    status_text.text(f"⚠️ Retry {retry+1}/{max_retries} for batch {batch_num + 1}. Waiting {wait_time}s...")
                ...
                if summary_response.status_code == 429:
                    status_text.text(f"⚠️ Rate limit for batch {batch_num + 1}. Retrying...")
                    continue
                ...
                status_text.text(f"✅ Batch {batch_num + 1} completed successfully")
                break
                ...
                if retry < max_retries - 1:
                    ...
                    st.warning(f"⚠️ Batch {batch_num + 1} failed after {max_retries} attempts. Skipping...")
    ...
    if len(articles) == 0:
        st.error("❌ No articles could be processed. The PubMed API may be rate limiting heavily.")
        st.info("💡 Please try again later or reduce your search query to get fewer articles.")
    else:
        st.success(f"✅ Successfully processed {len(articles)} out of {total_to_process} articles")
...
# ============================================================================
# MAIN APPLICATION - VERSIÓN CORREGIDA CON SEPARACIÓN DE VISTAS
# ============================================================================
...
def main():
    ...
    # ========================================================================
    # VISTA DE RESULTADOS (si ya hay una búsqueda completada)
    # ========================================================================
    if st.session_state.search_completed and st.session_state.results_generated:
        st.success(f"✅ ¡Búsqueda completada! {st.session_state.total_articles} artículos procesados exitosamente.")
        
        # Mostrar métricas
        col1, col2, col3, col4 = st.columns(4)
        with col1:
            st.metric("📊 Artículos encontrados", st.session_state.total_processed)
        with col2:
            st.metric("🔍 Después de filtro", st.session_state.total_articles)
        with col3:
            st.metric("⚙️ Threshold usado", f"{st.session_state.applied_threshold:.2f}")
        with col4:
            st.metric("🎨 Flavors generados", st.session_state.n_flavors)
        
        # Mostrar preview de flavors
        if st.session_state.flavors:
            display_flavors_preview(st.session_state.flavors)
        
        # ====================================================================
        # BOTONES DE DESCARGA - ¡AQUÍ ESTÁ LA SOLUCIÓN!
        # ====================================================================
        st.markdown("---")
        st.markdown("## 📥 Descargar Documentos")
        st.markdown("Tus documentos están listos para descargar:")
        ...
        with col_left:
            ...
            if st.session_state.docx_data is not None:
                ...
                st.caption(f"📄 Archivo: flavors_{timestamp}.docx")
            else:
                st.error("❌ Error: No se pudo generar el documento DOCX")
        ...
        with col_right:
            ...
            if st.session_state.csv_data is not None:
                ...
                st.caption(f"📊 Archivo: articles_{timestamp}.csv | Incluye todos los metadatos")
            else:
                st.error("❌ Error: No se pudo generar el archivo CSV")
        
        # Mostrar información de los artículos
        if st.session_state.articles:
            st.info(f"📊 El archivo CSV contiene {len(st.session_state.articles)} artículos con análisis completo")
        
        # Botón para nueva búsqueda
        st.markdown("---")
        col_btn1, col_btn2, col_btn3 = st.columns([1, 2, 1])
        with col_btn2:
            if st.button("🔄 INICIAR NUEVA BÚSQUEDA", use_container_width=True, type="secondary"):
                # Limpiar todo el estado
                for key in ['results_generated', 'search_completed', 'docx_data', 'csv_data', 
                           'articles', 'flavors', 'total_articles', 'total_processed', 
                           'n_flavors', 'applied_threshold', 'query_terms', 'hypothesis_terms']:
                    if key in st.session_state:
                        del st.session_state[key]
                st.rerun()
        
        return  # Salir para no mostrar los inputs
    
    # ========================================================================
    # VISTA DE CONFIGURACIÓN Y BÚSQUEDA (si no hay resultados)
    # ========================================================================
    ...
    with col2:
        st.info("""
        **Límite de artículos:**
        - Máximo 1000 artículos por búsqueda
        - Para mejor rendimiento
        - Procesamiento con delays para evitar rate limits
        """)
    ...
    if generate_button:
        if not query.strip():
            ...
            with st.spinner("🔍 Searching articles in PubMed..."):
                ...
                st.info(f"📊 Found {total_count} articles. Processing up to {len(id_list)} articles...")
            ...
            # Calcular relevancia
            with st.spinner("🧠 Calculating relevance with search and hypothesis..."):
            ...
            # Mostrar éxito y recargar para mostrar los botones
            st.balloons()
            st.success("🎉 Processing complete! Reloading to show download buttons...")
            time.sleep(2)
            st.rerun()
    ...
    st.markdown(
        ...
        <div style='text-align: center; color: gray; font-size: 0.8em;'>
            ...
            <strong>✅ Versión corregida - Los botones de descarga aparecen después de la búsqueda</strong>
        </div>
        """,
