...
def determine_flavor_aspect_and_difference(articles, flavor_name):
    """Determina el Aspecto y Diferencia de un flavor basado en sus características"""
    if not articles:
        return "Análisis general", "Enfoque integrador"
    ...
    # Determinar el aspecto principal
    aspect = ""
    if app_counts:
        top_app = app_counts.most_common(1)[0][0]
        if top_app == 'drug discovery':
            aspect = "Descubrimiento de fármacos y cribado virtual"
        elif top_app == 'vaccine development':
            aspect = "Desarrollo de vacunas e inmunoinformática"
        elif top_app == 'structural biology':
            aspect = "Biología estructural y dinámica molecular"
        elif top_app == 'evolutionary analysis':
            aspect = "Análisis filogenético y evolución viral"
        else:
            aspect = "Aplicación computacional"
    elif protein_counts:
        top_protein = protein_counts.most_common(1)[0][0]
        aspect = f"Estudios sobre {top_protein}"
    elif outcome_counts:
        top_outcome = outcome_counts.most_common(1)[0][0]
        aspect = f"Evaluación de {top_outcome}"
    elif method_counts:
        top_method = method_counts.most_common(1)[0][0]
        aspect = f"Aplicación de {top_method}"
    else:
        aspect = "Análisis computacional integrador"
    
    # Determinar la diferencia (lo que hace único a este flavor)
    difference = ""
    
    # Comparar con otros posibles flavors basado en características
    if app_counts.get('drug discovery', 0) > len(articles) * 0.4:
        difference = "Enfoque aplicado al descubrimiento de fármacos con énfasis en cribado virtual y validación in silico"
    elif app_counts.get('vaccine development', 0) > len(articles) * 0.3:
        difference = "Orientado al diseño racional de vacunas mediante vacunología reversa"
    elif protein_counts.get('VP40', 0) > len(articles) * 0.3:
        difference = "Centrado en la proteína VP40 y su papel en ensamblaje viral"
    elif protein_counts.get('VP35', 0) > len(articles) * 0.3:
        difference = "Centrado en la proteína VP35 y su función en replicación e inhibición de respuesta inmune"
    elif protein_counts.get('GP', 0) > len(articles) * 0.3:
        difference = "Centrado en la glicoproteína GP y su papel en entrada viral"
    elif outcome_counts.get('mortality', 0) > len(articles) * 0.3:
        difference = "Enfoque en outcomes clínicos de mortalidad y supervivencia"
    elif outcome_counts.get('efficacy', 0) > len(articles) * 0.3:
        difference = "Evaluación cuantitativa de eficacia y efectividad"
    elif 'mixed' in flavor_name.lower() or 'contradictory' in flavor_name.lower():
        difference = "Agrupa estudios con resultados contradictorios o heterogéneos"
    elif 'population' in flavor_name.lower():
        difference = "Focalizado en poblaciones específicas con características demográficas particulares"
    elif method_counts.get('machine learning', 0) > len(articles) * 0.5:
        difference = "Predomina el uso de algoritmos de machine learning para predicción y clasificación"
    else:
        difference = "Integra estudios con diversidad metodológica y temática"
...
def generate_flavor_summary_with_citations(articles, flavor_name, section='introduction'):
    ...
    # Determinar Aspecto y Diferencia
    ...
    # Contar tipos de estudio principales
    study_type_counts = Counter(study_types)
    top_study_types = [st for st, _ in study_type_counts.most_common(2)]
    
    # Generar encabezado con Aspecto y Diferencia
    header = f"### {flavor_name}\n\n"
    header += f"**🎯 Aspecto:** {aspect}\n\n"
    header += f"**🔍 Diferencia:** {difference}\n\n"
    header += "---\n\n"
    
    # Generar párrafo según la sección - AHORA MÁS EXTENSO
    if section == 'introduction':
        # Introducción más detallada
        summary = header
        
        if 'VP40' in main_topic or 'VP35' in main_topic or 'GP' in main_topic:
            summary += f"La caracterización bioinformática de las proteínas del virus Ebola, particularmente {main_topic}, ha sido abordada por {len(articles)} estudios computacionales que emplean técnicas avanzadas de machine learning, dinámica molecular y docking. "
            summary += f"Estos trabajos han revelado información crítica sobre la dinámica estructural, interacciones proteína-proteína y potencial como blanco terapéutico. "
            
            if key_articles:
                refs = [f"{auth} ({idx})" for idx, auth, _ in key_articles[:5]]
                if refs:
                    summary += f"Entre las contribuciones clave destacan los trabajos de {', '.join(refs)}, que han avanzado significativamente la comprensión de la función de {main_topic}. "
            
            if top_outcomes:
                summary += f"Los principales outcomes evaluados en estos estudios incluyen {', '.join(top_outcomes[:3])}, "
                summary += f"demostrando la relevancia clínica de estas investigaciones in silico. "
            
            if key_findings:
                find_refs = [f"{auth} ({idx})" for idx, auth, _ in key_findings[:3]]
                if find_refs:
                    summary += f"Hallazgos notables como los reportados por {', '.join(find_refs)} han identificado {key_findings[0][2] if key_findings else 'predicciones computacionales significativas'}, "
                    summary += f"estableciendo las bases para futuras validaciones experimentales."
            
            summary += f" La convergencia de estos {len(articles)} estudios demuestra la potencia de las aproximaciones computacionales para la caracterización de proteínas virales y la identificación de inhibidores potenciales."
        
        elif 'vaccine' in main_topic.lower():
            summary += f"El desarrollo de vacunas contra el virus Ebola ha recibido un impulso significativo mediante aproximaciones computacionales. Este flavor integra {len(articles)} estudios que emplean métodos bioinformáticos para predecir inmunogenicidad, identificar epítopes conservados y evaluar candidatos vacunales. "
            summary += f"La vacunología reversa y el modelado estructural han permitido diseñar constructos con mayor probabilidad de éxito experimental. "
            
            if key_articles:
                refs = [f"{auth} ({idx})" for idx, auth, _ in key_articles[:5]]
                if refs:
                    summary += f"Trabajos pioneros como los de {', '.join(refs)} han establecido marcos computacionales para el diseño racional de vacunas. "
            
            if top_outcomes:
                summary += f"Los outcomes clave evaluados en esta agrupación incluyen {', '.join(top_outcomes[:3])}, "
                summary += f"proporcionando métricas cuantitativas para la selección de candidatos. "
            
            summary += f"La integración de {len(articles)} estudios independientes fortalece la evidencia para los candidatos vacunales identificados y subraya el papel de la bioinformática en la aceleración del diseño de vacunas contra el virus Ebola."
        
        elif 'drug' in main_topic.lower() or 'discovery' in main_topic.lower():
            summary += f"Los esfuerzos de descubrimiento de fármacos contra el virus Ebola han sido transformados por aproximaciones computacionales. Este flavor agrupa {len(articles)} estudios que emplean cribado virtual, docking molecular, dinámica molecular y machine learning para identificar compuestos prometedores dirigidos contra proteínas virales clave. "
            summary += f"El reposicionamiento de fármacos y la identificación de nuevos compuestos han emergido como estrategias complementarias para acelerar el desarrollo terapéutico. "
            
            if key_articles:
                refs = [f"{auth} ({idx})" for idx, auth, _ in key_articles[:5]]
                if refs:
                    summary += f"Campañas de cribado notables como las realizadas por {', '.join(refs)} han identificado inhibidores con alta afinidad de unión. "
            
            if top_outcomes:
                summary += f"La eficacia, seguridad y toxicidad han sido los outcomes principales evaluados en estos {len(articles)} estudios, "
                summary += f"estableciendo un marco para la priorización de candidatos. "
            
            summary += f"La consistencia entre estos estudios computacionales sugiere blancos robustos para validación experimental y estrategias de reposicionamiento farmacológico."
        
        else:
            summary += f"Este flavor integra {len(articles)} estudios computacionales y bioinformáticos que investigan diversos aspectos de la biología del virus Ebola, función de proteínas e intervenciones terapéuticas. "
            summary += f"Las aproximaciones metodológicas incluyen machine learning, dinámica molecular, docking molecular y análisis filogenéticos. "
            
            if key_articles:
                refs = [f"{auth} ({idx})" for idx, auth, _ in key_articles[:5]]
                if refs:
                    summary += f"Investigaciones clave como las de {', '.join(refs)} han avanzado la comprensión fundamental de las proteínas virales. "
            
            if top_outcomes:
                summary += f"Los outcomes principales examinados incluyen {', '.join(top_outcomes[:3])}, "
                summary += f"reflejando el enfoque en resultados clínicamente relevantes. "
            
            summary += f"La diversidad de métodos computacionales empleados resalta tanto las fortalezas como las limitaciones de las aproximaciones bioinformáticas actuales en el estudio de patógenos emergentes."
    
    else:  # section == 'discussion'
        summary = header
        
        if 'VP40' in main_topic or 'VP35' in main_topic or 'GP' in main_topic:
            summary += f"Nuestros hallazgos sobre {main_topic} son consistentes con los reportados en {len(articles)} estudios computacionales previos que han elucidado características estructurales clave y dominios funcionales. "
            summary += f"La convergencia metodológica observada, particularmente en el uso de dinámica molecular y docking, ha permitido identificar regiones conservadas de alto valor para intervención terapéutica. "
            
            if key_articles:
                refs = [f"{auth} ({idx})" for idx, auth, _ in key_articles[:5]]
                if refs:
                    summary += f"Específicamente, {', '.join(refs)} reportaron resultados consistentes en cuanto a la dinámica estructural de {main_topic}, "
                    summary += f"validando las predicciones computacionales con datos experimentales complementarios. "
            
            if key_findings:
                find_refs = [f"{auth} ({idx})" for idx, auth, _ in key_findings[:3]]
                if find_refs:
                    summary += f"Los hallazgos cuantitativos de {', '.join(find_refs)} proporcionan evidencia adicional que respalda nuestras conclusiones sobre la importancia funcional de {main_topic}. "
            
            summary += f"Sin embargo, la heterogeneidad en las aproximaciones computacionales a través de estos {len(articles)} estudios sugiere que la estandarización de métodos podría mejorar la reproducibilidad y la comparabilidad de resultados. "
            summary += f"La integración de estos hallazgos con datos estructurales experimentales será crucial para avanzar hacia aplicaciones terapéuticas."
        
        elif 'vaccine' in main_topic.lower():
            summary += f"Nuestros resultados son consistentes con los estudios computacionales de desarrollo de vacunas que han identificado epítopes conservados en el virus Ebola. Este flavor incluye {len(articles)} investigaciones que han utilizado aproximaciones de vacunología reversa y modelado inmunoinformático. "
            summary += f"La predicción de epítopes de células T y B ha sido validada mediante estudios de acoplamiento molecular y dinámica de complejos inmunológicos. "
            
            if key_articles:
                refs = [f"{auth} ({idx})" for idx, auth, _ in key_articles[:5]]
                if refs:
                    summary += f"En particular, {', '.join(refs)} demostraron predicciones de inmunogenicidad similares, "
                    summary += f"con alta conservación entre cepas virales. "
            
            summary += f"La convergencia de hallazgos en {len(articles)} estudios computacionales independientes fortalece la evidencia para los candidatos vacunales identificados, "
            summary += f"proporcionando un marco robusto para la validación experimental preclínica y eventual traducción clínica."
        
        elif 'drug' in main_topic.lower() or 'discovery' in main_topic.lower():
            summary += f"Nuestros hallazgos en el descubrimiento de fármacos se alinean con los reportados en {len(articles)} estudios de cribado in silico que han identificado inhibidores potenciales contra proteínas del virus Ebola. "
            summary += f"Las aproximaciones basadas en estructura han permitido identificar sitios de unión conservados con alta relevancia farmacológica. "
            
            if key_articles:
                refs = [f"{auth} ({idx})" for idx, auth, _ in key_articles[:5]]
                if refs:
                    summary += f"Notablemente, {', '.join(refs)} reportaron afinidades de unión y patrones de interacción molecular similares, "
                    summary += f"sugiriendo blancos robustos para desarrollo terapéutico. "
            
            summary += f"La consistencia a través de estos {len(articles)} estudios computacionales sugiere blancos robustos para validación experimental. "
            summary += f"El reposicionamiento de fármacos aprobados por FDA ha emergido como una estrategia complementaria que podría acelerar el desarrollo clínico, reduciendo los tiempos y costos asociados."
        
        else:
            summary += f"Nuestro análisis integra hallazgos de {len(articles)} estudios computacionales que han investigado la biología del virus Ebola a través de diversas aproximaciones in silico. "
            summary += f"La diversidad de métodos computacionales empleados resalta tanto las fortalezas como las limitaciones de las aproximaciones bioinformáticas actuales. "
            summary += f"Estudios de dinámica molecular, docking y machine learning han proporcionado información complementaria sobre la función viral. "
            
            if key_articles:
                refs = [f"{auth} ({idx})" for idx, auth, _ in key_articles[:5]]
                if refs:
                    summary += f"Contribuciones clave de {', '.join(refs)} proporcionan contexto para nuestros hallazgos, "
                    summary += f"estableciendo las bases para investigaciones futuras. "
            
            summary += f"La integración de datos computacionales con evidencia experimental será esencial para validar predicciones y traducir el conocimiento bioinformático a aplicaciones clínicas efectivas contra el virus Ebola."
...
def merge_small_clusters(flavors, target_count=4):
    """Fusiona clusters pequeños para obtener aproximadamente target_count flavors grandes"""
    if not flavors:
        return []
    ...
    if remaining_articles:
        ...
        # Extraer outcomes principales del grupo fusionado
        all_outcomes = []
        for article in remaining_articles:
...
def discover_flavors_by_outcomes(articles):
    ...
    try:
        ...
        # Reducir número de clusters para obtener grupos más grandes
...
def generate_all_flavors(articles, hypothesis):
    """Genera todos los flavors desde múltiples perspectivas y los fusiona"""
    if not articles:
        return {}
    
    all_flavors = []
    
    # Generar clusters desde diferentes perspectivas
    ...
    # Si no hay suficientes clusters, crear uno con todos los artículos
    if not all_flavors:
        all_flavors = [{
            'type': 'default',
            'id': 'default_cluster',
    ...
    # Fusionar clusters pequeños para obtener 3-4 flavors grandes
    merged_flavors = merge_small_clusters(all_flavors, target_count=4)
    
    # Organizar por tipo para mantener estructura en el documento
    flavors_by_category = {
        'semantic_clusters': [],
        'outcome_clusters': [],
        'effect_clusters': [],
...
def main():
    ...
    st.info("⚡ Versión mejorada: Hasta 1000 artículos | Flavors fusionados con Aspecto y Diferencia")
    ...
    **Características:**
    ...
    - 📊 **Clustering y fusión**: Se generan 3-4 flavors grandes con 10-15 referencias cada uno
    - 📝 **Resúmenes extensos con referencias insertadas**: Párrafos de al menos 5-7 líneas
    - 🎯 **Aspecto y Diferencia**: Cada flavor incluye encabezado con su característica principal y valor diferencial
    ...
    if generate_button:
        if not query.strip():
            ...
            if len(articles) < 5:
                st.error("❌ No hay suficientes artículos para generar flavors (mínimo 5)")
                st.stop()
            
            with st.spinner("🔍 Descubriendo y fusionando flavors (3-4 grupos grandes)..."):
                flavors = generate_all_flavors(articles, hypothesis)
            
            total_flavors = sum(len(flavor_list) for flavor_list in flavors.values())
            st.success(f"✅ Generados {total_flavors} flavors grandes desde {len(articles)} artículos")
            
            with st.spinner("📄 Creando documento con resúmenes extensos y referencias insertadas..."):
    ...
    if st.session_state.docx_generated and st.session_state.docx_data is not None:
        ...
        with col3:
            st.metric("Flavors grandes", st.session_state.n_flavors)
        ...
        with st.expander("📖 Guía de uso"):
            ...
            **Resumen de la ejecución:**
            ...
            - Flavors grandes generados: {st.session_state.n_flavors}
            
            **Cómo ajustar los resultados:**
            - Si obtienes **pocos artículos** (<15), **reduce el umbral de relevancia** (ej. 0.25-0.30)
            ...
            **Estructura del documento:**
            1. **FLAVORS PARA INTRODUCCIÓN**: Párrafos extensos (5+ líneas) con referencias insertadas
            2. **FLAVORS PARA DISCUSIÓN**: Párrafos extensos (5+ líneas) con referencias insertadas
            
            **Novedad:** Cada flavor incluye:
            - **🎯 Aspecto**: La característica principal que define este grupo de estudios
            - **🔍 Diferencia**: Lo que hace único a este flavor frente a otros
            
            Estos elementos permiten contextualizar rápidamente cada flavor y entender su valor diferencial.
            """)
    ...
    st.markdown(
        ...
        <div style='text-align: center; color: gray; font-size: 0.8em;'>
            🧠 PubMed AI Analyzer - Generador de Flavors Avanzado v11.0<br>
            BioBERT NER | HDBSCAN + Fusión de clusters | Aspecto + Diferencia por flavor | 3-4 flavors grandes con ~15 referencias
        </div>
        """,
