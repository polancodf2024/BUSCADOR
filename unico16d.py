...
# ============================================================================
# FUNCIONES DE EXTRACCIÓN DE ENTIDADES (resumidas por brevedad)
# ============================================================================
...
def search_pubmed(query, retmax=1000):
    """Search articles in PubMed - SUPPORTS UP TO 1000"""
    base_url = "https://eutils.ncbi.nlm.nih.gov/entrez/eutils/"
...
# ============================================================================
# MEJORA: DETERMINACIÓN AVANZADA DE ASPECTO Y DIFERENCIA
# ============================================================================
...
# ============================================================================
# MEJORA: ASIGNACIÓN EXCLUSIVA DE ARTÍCULOS
# ============================================================================
...
def assign_articles_to_best_flavor(flavors):
    ...
    all_articles = []
    for flavor in flavors:
        all_articles.extend(flavor['articles'])
...
# ============================================================================
# NUEVA FUNCIÓN: GENERAR RESUMEN EJECUTIVO
# ============================================================================

def generate_executive_summary(flavors_by_category, total_articles, query, hypothesis, relevance_threshold):
    """Generate an executive summary with key metrics and top flavors"""
    
    # Collect all flavors from all categories
    all_flavors = []
    for category, flavors in flavors_by_category.items():
        all_flavors.extend(flavors)
    
    if not all_flavors:
        return "No flavors generated."
    
    # Sort by number of articles (largest first)
    all_flavors.sort(key=lambda x: x['n_articles'], reverse=True)
    top_flavors = all_flavors[:4]
    
    summary = "## EXECUTIVE SUMMARY\n\n"
    summary += "### Overview\n\n"
    summary += f"- **Total articles analyzed:** {total_articles}\n"
    summary += f"- **Flavors generated:** {len(all_flavors)}\n"
    summary += f"- **Relevance threshold:** {relevance_threshold}\n"
    summary += f"- **Search strategy:** {query[:150]}...\n"
    summary += f"- **Hypothesis:** {hypothesis[:150]}...\n\n"
    
    # Embedder info
    if USE_FALLBACK:
        summary += f"- **Embedding model:** SBERT (fallback - BioBERT unavailable)\n"
    elif AI_EMBEDDINGS_AVAILABLE:
        summary += f"- **Embedding model:** BioBERT (biomedical optimized)\n"
    else:
        summary += f"- **Embedding model:** TF-IDF + LSA (fallback)\n"
    summary += "\n"
    
    summary += "### Top Flavors\n\n"
    summary += "The following are the most significant thematic groups identified in the literature:\n\n"
    
    for i, flavor in enumerate(top_flavors, 1):
        # Get aspect and difference for this flavor
        aspect, difference = determine_flavor_aspect_and_difference(
            flavor.get('representative_articles', flavor['articles'][:5]), 
            flavor['name']
        )
        
        # Calculate average quality score for this flavor
        quality_scores = [a.get('quality_score', 0) for a in flavor['articles']]
        avg_quality = sum(quality_scores) / len(quality_scores) if quality_scores else 0
        
        # Extract main outcomes
        all_outcomes = []
        for a in flavor['articles']:
        ...
        for a in flavor['articles']:
            ...
            if 'gp' in title:
                all_proteins.append('GP')
        protein_counts = Counter(all_proteins)
        top_proteins = [p for p, _ in protein_counts.most_common(3)]
        
        summary += f"#### Flavor {i}: {flavor['name']}\n\n"
        summary += f"**Size:** {flavor['n_articles']} articles | **Avg. Quality:** {avg_quality:.1f}/100\n\n"
        summary += f"**🎯 Aspect:** {aspect}\n\n"
        summary += f"**🔍 Difference:** {difference}\n\n"
        
        if top_outcomes:
            summary += f"**Key Outcomes:** {', '.join(top_outcomes)}\n\n"
        if top_proteins:
            summary += f"**Key Proteins:** {', '.join(top_proteins)}\n\n"
        
        # List key authors (top 3 by quality)
        top_authors = []
        for a in flavor['representative_articles'][:3]:
            authors = a.get('authors', '').split(',')[0] if a.get('authors') else 'Author'
            top_authors.append(authors)
        if top_authors:
            summary += f"**Key Contributors:** {', '.join(top_authors)}\n\n"
...
# ============================================================================
# NUEVA FUNCIÓN: GENERAR TABLA COMPARATIVA DE FLAVORS
# ============================================================================

def generate_flavor_comparison_table(flavors_by_category):
    """Generate a comparison table of all flavors with key metrics"""
    
    # Collect all flavors
    all_flavors = []
    for category, flavors in flavors_by_category.items():
        for flavor in flavors:
            # Get aspect and difference
            aspect, difference = determine_flavor_aspect_and_difference(
                flavor.get('representative_articles', flavor['articles'][:5]), 
                flavor['name']
            )
            ...
            # Extract main study types
            study_types = []
            for a in flavor['articles']:
                st = a.get('study_types', '')
                if st and st != 'Not specified':
            ...
            all_flavors.append({
                'name': flavor['name'],
                'size': flavor['n_articles'],
                'avg_quality': avg_quality,
                'aspect': aspect,
                'difference': difference,
                'top_outcomes': top_outcomes,
                'study_types': top_study_types
            })
    
    if not all_flavors:
        return "No flavors available for comparison."
    
    # Sort by size (largest first)
    all_flavors.sort(key=lambda x: x['size'], reverse=True)
    
    table = "## FLAVOR COMPARISON TABLE\n\n"
    table += "| Flavor | Size | Avg Quality | Aspect | Key Outcomes | Study Types |\n"
    table += "|--------|------|-------------|--------|--------------|-------------|\n"
    
    for flavor in all_flavors:
        name = flavor['name'][:40] + "..." if len(flavor['name']) > 40 else flavor['name']
        size = flavor['size']
        quality = f"{flavor['avg_quality']:.1f}"
        aspect = flavor['aspect'][:35] + "..." if len(flavor['aspect']) > 35 else flavor['aspect']
        outcomes = ', '.join(flavor['top_outcomes'][:2]) if flavor['top_outcomes'] else "Not specified"
        outcomes = outcomes[:30] + "..." if len(outcomes) > 30 else outcomes
        study_types = ', '.join(flavor['study_types'][:2]) if flavor['study_types'] else "Various"
        
        table += f"| {name} | {size} | {quality} | {aspect} | {outcomes} | {study_types} |\n"
    
    table += "\n"
    table += "*Table 1. Comparison of thematic flavors identified in the literature.*\n\n"
    table += "**Abbreviations:** Quality = average quality score (0-100) based on study design, sample size, and methodological rigor.\n\n"
    
    return table

# ============================================================================
# FUNCIÓN PARA GENERAR FLAVOR SUMMARY CON CITAS
# ============================================================================
...
# ============================================================================
# FUNCIÓN PARA CREAR DOCUMENTO COMPLETO CON NUEVAS SECCIONES
# ============================================================================

def create_document_with_flavors(flavors, hypothesis, query, total_articles, relevance_threshold):
    """Create a DOCX document with executive summary, comparison table, and flavors"""
    doc = Document()
    ...
    # EXECUTIVE SUMMARY
    # ========================================================================
    executive_summary = generate_executive_summary(flavors, total_articles, query, hypothesis, relevance_threshold)
    ...
    # ========================================================================
    comparison_table = generate_flavor_comparison_table(flavors)
    ...
    for category_name, flavor_list in flavors.items():
        ...
        for flavor in flavor_list:
            ...
            # Parse and add summary with formatting
            for line in intro_summary.split('\n'):
                if line.startswith('###'):
                    doc.add_heading(line[4:], level=3)
    ...
    for category_name, flavor_list in flavors.items():
        ...
        for flavor in flavor_list:
            ...
            for line in disc_summary.split('\n'):
                if line.startswith('###'):
                    doc.add_heading(line[4:], level=3)
                elif line.startswith('**🎯 Aspect:**'):
                    p = doc.add_paragraph()
...
def main():
    ...
    st.info("⚡ Enhanced version: Executive Summary | Comparison Table | Up to 1000 articles | Exclusive assignment | Aspect + Difference per flavor")
    ...
    **Features:**
    ...
    - 📋 **Executive Summary**: Overview of top flavors with key metrics
    - 📊 **Comparison Table**: Tabular comparison of all flavors by size, quality, outcomes
    ...
    if generate_button:
        if not query.strip():
            ...
            with st.spinner("📄 Creating document with Executive Summary, Comparison Table, and extended summaries..."):
    ...
    if st.session_state.docx_generated and st.session_state.docx_data is not None:
        ...
        st.download_button(
            label="📥 DOWNLOAD FLAVORS (Executive Summary + Comparison Table + Intro + Discussion)",
        ...
        with st.expander("📖 Usage Guide"):
            ...
            **Document Structure:**
            1. **EXECUTIVE SUMMARY**: Overview of top flavors with key metrics
            2. **FLAVOR COMPARISON TABLE**: Tabular comparison of all flavors
            3. **FLAVORS FOR INTRODUCTION**: Extended paragraphs (5+ lines) with embedded citations
            4. **FLAVORS FOR DISCUSSION**: Extended paragraphs (5+ lines) with embedded citations
            
            **New Features:**
            - **Executive Summary**: Quick overview of the most significant thematic groups
            - **Comparison Table**: Side-by-side comparison of all flavors by size, quality, outcomes, and study types
    ...
    st.markdown(
        ...
        <div style='text-align: center; color: gray; font-size: 0.8em;'>
            🧠 PubMed AI Analyzer - Advanced Flavor Generator v13.0<br>
            Executive Summary | Comparison Table | BioBERT → SBERT → TF-IDF Fallback<br>
            Exclusive Assignment | Aspect + Difference per Flavor | Up to 1000 articles | English output
        </div>
        """,
