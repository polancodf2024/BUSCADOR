...
def generate_flavor_summary_with_citations(articles, flavor_name, section='introduction'):
    ...
    # Generar párrafo según la sección
    if section == 'introduction':
        ...
        if 'VP40' in main_topic or 'VP35' in main_topic or 'GP' in main_topic:
            ...
            summary += f" However, the heterogeneity in computational approaches across these {len(articles)} studies suggests that standardization of methods may improve reproducibility."
            ...
            summary += f" The convergence of findings across {len(articles)} independent computational studies strengthens the evidence for the identified vaccine candidates."
            ...
            summary += f" The consistency across {len(articles)} computational studies suggests robust targets for experimental validation."
        
        else:
            summary = f"Our analysis integrates findings from {len(articles)} computational studies that have investigated Ebola virus biology through various in silico approaches."
...
def create_document_with_flavors(flavors, hypothesis, query, total_articles):
    ...
    for category_name, flavor_list in flavors.items():
        ...
        for flavor in flavor_list:
            # Generar resumen para introducción con citas insertadas
            ...
            # Párrafo con referencias insertadas
    ...
    for category_name, flavor_list in flavors.items():
        ...
        for flavor in flavor_list:
            # Generar resumen para discusión con citas insertadas
...
def main():
    ...
    st.info("⚡ Versión mejorada: Resúmenes con referencias insertadas | Separación Introducción/Discusión")
    ...
    **Características:**
    ...
    - 📝 **Resúmenes con referencias insertadas**: Las citas aparecen en el texto
    - 📚 **Separación Introducción/Discusión**: Párrafos específicos para cada sección
    - 📄 **Formato Vancouver**: Referencias bibliográficas estándar
    """)
    ...
    with col_b:
        st.info("📄 Salida: DOCX con resúmenes + referencias insertadas")
    ...
    if st.session_state.docx_generated and st.session_state.docx_data is not None:
        ...
        with st.expander("📖 Estructura del documento"):
            ...
            **Estructura del documento generado:**
            ...
            1. **FLAVORS PARA INTRODUCCIÓN**
               - Párrafos con referencias insertadas (ej: "Key contributions from Smith (1) and Jones (2)...")
               - Diseñados para establecer el estado del arte
               - Justifican la necesidad de la investigación
            
            2. **FLAVORS PARA DISCUSIÓN**
               - Párrafos con referencias insertadas
               - Comparan resultados con la literatura
               - Contextualizan los hallazgos
            
            **Ejemplo de referencia insertada:**
            > "Key contributions from Smith (1) and Jones (2) have advanced our understanding of VP40 function. Notable findings include Johnson (3) demonstrating significant computational predictions."
            
            Luego, las referencias completas aparecen al final en formato Vancouver.
            """)
    ...
    st.markdown(
        ...
        <div style='text-align: center; color: gray; font-size: 0.8em;'>
            🧠 PubMed AI Analyzer - Generador de Flavors Avanzado v8.0<br>
            BioBERT NER | HDBSCAN Clustering | Referencias insertadas | Separación Intro/Disc
        </div>
        """,
