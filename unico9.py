...
# ============================================================================
# CONFIGURACIÓN DE MODO AVANZADO (INTENTAR CARGAR MÓDULOS OPCIONALES)
# ============================================================================
...
try:
    ...
    print("⚠️ sentence-transformers no instalado. Modo embeddings no disponible")
...
try:
    ...
    models_to_try = [
        "en_core_web_sm",      # Modelo pequeño (más rápido)
    ...
    for model_name in models_to_try:
        try:
            ...
            print(f"✅ spaCy cargado con modelo: {model_name}")
            break
        except:
            continue
    ...
    if NLP_MODEL is not None:
        ...
        print("⚠️ No se pudo cargar ningún modelo spaCy")
        DEPENDENCY_PARSING_AVAILABLE = False
        
except ImportError:
    print("⚠️ spacy no instalado. Dependency parsing no disponible")
    DEPENDENCY_PARSING_AVAILABLE = False
    NLP_MODEL = None
except Exception as e:
    print(f"⚠️ Error cargando modelo spaCy: {e}")
    DEPENDENCY_PARSING_AVAILABLE = False
    NLP_MODEL = None
...
def analyze_sentiment_by_outcome(text):
    ...
    for outcome in all_outcomes:
        ...
        start = max(0, outcome_index - 200)  # Aumentado a 200 para mejor contexto
        ...
        for term in positive_terms:
            if term in context:
                ...
                if any(neg in prev_words for neg in negation_words):
                    sentiment_score -= 1  # Negación invierte el sentido
                else:
                    sentiment_score += 1
...
def extract_semantic_relations(text):
    """
    Extrae relaciones semánticas usando dependency parsing
    Identifica: [sujeto] - [verbo] - [objeto] con contexto médico
    """
    ...
    try:
            ...
            if token.pos_ == "VERB" and token.lemma_ in medical_verbs:
                ...
                for child in token.children:
                    if child.dep_ in ["nsubj", "nsubjpass"]:
                        # Obtener la frase completa del sujeto
        ...
        return relations[:10]  # Limitar a 10 relaciones
    except Exception as e:
        return [{'error': str(e)}]
...
def get_article_references(pmid):
    ...
    try:
        ...
        return references[:50]  # Limitar a 50 referencias
    except:
        return []
...
def build_citation_network(articles):
    ...
    for i, article in enumerate(articles):
        ...
        # Simular algunas citas basadas en años y relevancia
        citations = []
        ...
        # Los artículos más recientes citan a los más antiguos (simplificado)
        for j, other in enumerate(articles):
            ...
            # Si el otro es más antiguo, podría ser citado (simulación)
...
def extract_coauthors(authors_string):
    ...
    # Eliminar "et al." si existe
    authors = [a for a in authors if a.lower() not in ['et al.', 'et al']]
    
    return authors[:10]  # Limitar a 10 coautores
...
def perform_meta_analysis(articles_by_outcome):
    ...
    for outcome, articles in articles_by_outcome.items():
        ...
        for article in articles:
            ...
            if numeric_str:
                # Buscar patrones como "HR=0.84 (0.75-0.95)"
        ...
        # Calcular meta-análisis simple (efecto fijo)
        effects = np.array(effects)
        weights = np.ones_like(effects)  # Pesos iguales simplificados
        
        # Efecto combinado (media ponderada simple)
        combined_effect = np.average(effects, weights=weights)
        
        # Calcular heterogeneidad (I² simplificado)
...
def extract_all_outcomes(text):
    ...
    # Patrones genéricos para detectar outcomes
    outcome_patterns = [
...
def calculate_grade_level(study_type, quality_score, consistency, precision):
    ...
    """
    # Mapeo de tipos de estudio a niveles GRADE base
    grade_base = {
        'Meta-análisis': 'ALTO',
        'RCT': 'ALTO',
        'Cohorte': 'MODERADO',
    ...
    # Encontrar el tipo de estudio (puede haber múltiples)
    base_level = 'MUY BAJO'
    if study_type:
    ...
    # Penalizaciones (downgrades)
    downgrades = 0
    
    # Riesgo de sesgo (basado en calidad)
    if quality_score < 50:
        downgrades += 1
    if quality_score < 30:
        downgrades += 1
...
def aggregate_evidence_by_outcome(all_articles):
    ...
    for article in all_articles:
        ...
        if outcomes_analysis and outcomes_analysis != '':
            # Extraer outcomes de este artículo
...
def enhanced_quality_scoring(ai_analysis, full_text):
    ...
    # 4. DETECCIÓN DE CIEGO/ENMASCARAMIENTO
...
def analyze_methodological_quality(text):
    ...
    # 1. Tipo de estudio por patrones específicos
    study_patterns = {
    ...
    # 2. Factores de calidad adicionales
    quality_factors = []
...
def calculate_evidence_strength(statistical_results, quality_score, sample_size=None):
    ...
    # Combinar con calidad del estudio
...
def analyze_article_with_ai(title, abstract):
    ...
    # Obtener tamaño muestral
    sample_size = methodological_quality.get('sample_size', '')
    
    # Calcular fuerza de evidencia CON DIRECCIONALIDAD Y PRECISIÓN
    ...
    # Análisis de outcomes (MEJORADO con detección de negación)
    ...
    # 1. Análisis semántico con embeddings (si disponible)
    semantic_analysis = {}
    ...
    # Añadir riesgo de sesgo si está disponible
    ...
    # Formatear resultados numéricos para CSV
    numeric_summary = ""
    if numeric_results:
        numeric_parts = []
...
def search_pubmed_with_ai(query, retmax=1000):
    ...
    try:
        ...
        for batch_num in range(num_batches):
            ...
            try:
                ...
                for j, doc_sum in enumerate(summary_root.findall(".//DocSum")):
                    ...
                    # Analizar con IA MEJORADA (con funciones avanzadas)
                    ...
                    # NUEVAS COLUMNAS DE EVALUACIÓN MEJORADA
        ...
        # ============================================================================
        # ANÁLISIS AGREGADO POR OUTCOMES (MEJORAS 1 y 4)
        # ============================================================================
        
        if articles:
            # Agrupar artículos por outcome
            outcomes_dict = aggregate_evidence_by_outcome(articles)
            
            # Calcular consistencia para cada outcome
            ...
            # Calcular nivel GRADE y métricas de red para cada artículo
            for article in articles:
                ...
                for outcome in article_outcomes[:3]:
                    ...
                    # Determinar precisión para este artículo
                ...
                # Añadir meta-análisis si aplica
...
def main():
    ...
    **La IA analiza cada artículo y extrae:**
    ...
    - ✅ **NUEVO: Direccionalidad del efecto (protege/daña)**
    - ✅ **NUEVO: Consistencia entre estudios**
    - ✅ **NUEVO: Nivel GRADE de evidencia**
    - ✅ **NUEVO: Precisión de intervalos de confianza**
    - ✅ **NUEVO: Análisis semántico con BioBERT** (si instalado)
    - ✅ **NUEVO: Relaciones semánticas** (si instalado)
    - ✅ **NUEVO: Riesgo de sesgo Cochrane**
    - ✅ **NUEVO: Meta-análisis automático**
    - ✅ **NUEVO: Red de citas y coautorías**
    ...
    **El CSV incluye TODA esta información para cada artículo**
    ...
    with col_a:
        if AI_EMBEDDINGS_AVAILABLE:
            ...
            st.info("ℹ️ BioBERT: pip install sentence-transformers")
    with col_b:
    ...
    with col_c:
        st.info("⚡ Modo avanzado automático")
    ...
    with st.expander("ℹ️ ¿Qué significa cada columna del CSV?"):
        ...
        | `ANALISIS_SEMANTICO` | **NUEVO:** Análisis con BioBERT |
        | `RELACIONES_SEMANTICAS` | **NUEVO:** Relaciones causales |
        | `RIESGO_SESGO` | **NUEVO:** Riesgo de sesgo Cochrane |
        | `META_ANALISIS` | **NUEVO:** Resultados de meta-análisis |
        | `NUM_CITAS` | **NUEVO:** Número de citas recibidas |
        | `NUM_COAUTORES` | **NUEVO:** Número de coautores |
        ...
        if not query.strip():
            ...
            if articles:
                ...
                columns_order = [
                    ...
                    'semantic_analysis', 'semantic_relations', 'risk_of_bias',
                ...
                column_names = {
                    ...
                    'semantic_relations': 'RELACIONES_SEMANTICAS',
    ...
    if st.session_state.csv_generated and st.session_state.df_results is not None:
        ...
        with st.expander("📋 Ver estructura completa del CSV"):
            ...
            for col in st.session_state.df_results.columns:
                if col in ['ANALISIS_SEMANTICO', 'RELACIONES_SEMANTICAS', 'RIESGO_SESGO', 'META_ANALISIS', 'NUM_CITAS', 'NUM_COAUTORES']:
                    st.write(f"  **• {col}** ⬅️ NUEVA COLUMNA AVANZADA")
    ...
    st.markdown(
        ...
        <div style='text-align: center; color: gray; font-size: 0.8em;'>
            🧠 PubMed AI Analyzer v4.0 - Lectura Crítica Avanzada con BioBERT, GRADE, Meta-análisis y Redes de Citas | Totalmente genérico
        </div>
        """,
