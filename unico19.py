...
# ============================================================================
# FUNCIÓN CORREGIDA: FUSIÓN DE FLAVORS SIMILARES
# ============================================================================
...
def merge_similar_flavors(flavors, similarity_threshold=0.7):
    ...
    for i, flavor1 in enumerate(flavors):
        ...
        # Usar PMIDs para comparar artículos (son hashables)
        current_pmids = set()
        for a in flavor1.get('articles', []):
            pmid = a.get('pmid', str(id(a)))  # Usar PMID o ID único como string
        ...
        for j, flavor2 in enumerate(flavors):
            ...
            # Calcular intersección de artículos usando PMIDs
            articles2_pmids = set()
            for a in flavor2.get('articles', []):
                pmid = a.get('pmid', str(id(a)))
                articles2_pmids.add(pmid)
            
            common_articles = current_pmids & articles2_pmids
            article_similarity = len(common_articles) / max(len(current_pmids), len(articles2_pmids), 1)
            
            # Si son suficientemente similares, fusionar
            if keyword_similarity > similarity_threshold or article_similarity > 0.5:
                # Fusionar artículos (usando PMIDs para evitar duplicados)
                current_pmids = current_pmids | articles2_pmids
                used.add(j)
        
        # Reconstruir la lista de artículos fusionados
        all_articles_dict = {}
        all_articles_list = list(flavor1.get('articles', []))
        for k in used:
            if k != i and k < len(flavors):
                all_articles_list.extend(flavors[k].get('articles', []))
        
        for article in all_articles_list:
            pmid = article.get('pmid', str(id(article)))
            if pmid not in all_articles_dict:
                all_articles_dict[pmid] = article
